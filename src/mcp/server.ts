...
server.registerTool(
  'get_week_report',
  {
    title: '获取本周周报',
    description:
      '获取本周的周报内容（ISO 8601 周，周一为一周开始）。当用户询问本周总结、本周周报时使用。注意：用户说"本周的日报/这周每天做了什么"是要日报列表，应使用 get_week_dailies，不是本工具。如果本周还没有周报，返回明确的未找到结果。',
  },
  async () =>
    guard(async () => {
...
server.registerTool(
  'get_week_dailies',
  {
    title: '获取本周的所有日报',
    description:
      '获取本周（周一至周日）的全部日报列表。当用户询问本周每天都做了什么、查看本周/这周的日报、或需要汇总本周日报来写周报时使用。注意：用户要的是日报列表而非周报；要周报请用 get_week_report。返回按日期升序的日报数组，以及 missingDates（周一至今还没有日报的日期，可据此提醒用户补写）；本周还没有任何日报时返回空列表。',
  },
  async () =>
    guard(async () => {
